...
$path_aeit_library = \Drupal::configFactory()->getEditable('sabc_private_data_form.settings')->get('path_aeit_library');
    ...
    private $pathAeitLibrary;

		function __construct($v=TRUE){
			parent::__construct($v);
      $this->pathAeitLibrary = \Drupal::configFactory()->getEditable('sabc_private_data_form.settings')->get('path_bccgg_file');
		}

		function fnGetSchools($designationType = 'All', $additionalDetails = 'false', $accreditation = 'false'){

			//CALL THE APPROPRIATE WEB SERVICE
			$this->WSDL = fnWS('WS-HOSTS', 'GET_SCHOOLS');
			$ws = $this->fnRequest('getSchoolList', array(
		...
		function fnGetGrantSchools(){

      if (($handle = fopen(DRUPAL_ROOT . $this->pathAeitLibrary, 'rb')) !== FALSE) {
				$nn = 0;
      ...
      return $bigArray;
		...
		function getGrantSchoolDetails(){
			$school_info = "";
			@$url = $_GET["q"];
			$pieces = explode("/", $url);
			$searchfor = $pieces[1];
      ...
      foreach ($arrayOschools as $k => $v){
		...
		function fnGetSchoolDetails($schoolIDX){
		...
		function fnValidateSchoolCode($sc){
